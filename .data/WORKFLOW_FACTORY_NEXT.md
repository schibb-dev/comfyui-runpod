...
**Next session focus:** optional hourly pick by `rating_effective` (after G); Discovery index watcher — see [`docs/RATINGS_V1_PLAN.md`](../docs/RATINGS_V1_PLAN.md). **Clip bind for hourlies** (★ + newer, sequenced separately from policy-as-config): [`docs/HOURLY_CLIP_GUIDANCE_PLAN.md`](../docs/HOURLY_CLIP_GUIDANCE_PLAN.md).
