...
7. **Workbench (current):** phone main page is the **job list**. Tap opens that job **fullscreen**. **Up / down** stays inside the section you opened (Comfy Queue, Pending, Errors, or Completed) and snaps to the next job. **Swipe left** opens details, **swipe right** opens actions. **← Back to list** returns to the list. **Autoplay** and **Loop** toggles sit on the swipe bar. Working set, search, and filters stay in hamburger → This screen. Desktop keeps the two-column tools/list/detail chrome.
