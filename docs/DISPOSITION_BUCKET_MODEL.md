...
**Phase 2 (planned → this seam):** Hourly / pools bind Use via the clip model (★ lottery + newer bias), not a separate “prefer default” flag — see [HOURLY_UTILITY_PLAN.md](./HOURLY_UTILITY_PLAN.md) (U0 / U2).
...
**Asset / Clip / Use (north star).** Operators should browse and queue from **clips** in Discovery (Phase 1: clip rail + Queue from clip; Phase 3: `/discovery/clips` edit + browse), not from whole-file scrubber side-effects. Resolve order on the factory side is already use → source_clip → default_clip → sibling → sidecar → full. Hourly guidance (★ + newer) is sequenced in [HOURLY_UTILITY_PLAN.md](./HOURLY_UTILITY_PLAN.md); clip-centric ratings remain later.

---

## See also

- [HOURLY_UTILITY_PLAN.md](./HOURLY_UTILITY_PLAN.md) — hourlies as ruleset (policy + ★/newer bind; Disposition Phase 2 seam)
