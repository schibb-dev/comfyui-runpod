...
**Companion:** provenance and durable graph concepts live in [`LINEAGE_INDEX_SKETCH.md`](./LINEAGE_INDEX_SKETCH.md). Lineage and similarity are **separate capabilities** that meet **at the selected asset**. **Inferred ratings** (workflow/source/recipe scores from downstream keepers) are specified in [`RATINGS_V1_PLAN.md`](./RATINGS_V1_PLAN.md). **All programs (queue, orchestration, infra, etc.):** [`PLANNING_OVERVIEW.md`](./PLANNING_OVERVIEW.md).
